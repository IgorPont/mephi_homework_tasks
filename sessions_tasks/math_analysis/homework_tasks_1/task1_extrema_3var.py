...
def main() -> None:
    ...
    # 1. Объявляем переменные (вещественные числа) и задаем исходную функцию
    ...
    print("\n=== Анализ Гессе для каждой точки ===")
    for s in sol:
        # Подстановка найденной точки в символическую матрицу. Получаем числовую матрицу
        H_num = H.subs(s)
        # H_num.eigenvals() возвращает словарь {собственное_значение: кратность}
        eigvals = [float(ev) for ev in H_num.eigenvals().keys()]
        # Округляем только для вывода, классификацию оставляем на исходных float(ev)
        eigvals_rounded = [round(ev, 4) for ev in eigvals]

        # Классификация по признакам матрицы Гессе
        if all(ev > 0 for ev in eigvals):
            # Если все ev > 0 -> квадратичная форма положительно определена -> локальный минимум.
            classification = "локальный минимум"
        elif all(ev < 0 for ev in eigvals):
            # Если все ev < 0 -> отрицательно определена -> локальный максимум.
            classification = "локальный максимум"
        else:
            # Если есть разные знаки -> форма не определена -> седло.
